...
| Benchmark | Current Best Ours (Run7) | jina-embeddings-v4 (3.8B) | TomoroAI (8B) | Gap to Best |
|-----------|--------------------------|----------------------------|---------------|-------------|
| KoViDoRe V1 (NDCG@5) | 0.7115 | 0.8650 | 0.7920 | -0.1535 vs jina |
| KoViDoRe V2 (NDCG@10) | 0.4339 | 0.5500 | 0.4380 | -0.1161 vs jina |

Run7 기준으로 V1/V2 모두 현재까지 최고 성능을 달성했지만, 여전히 jina-embeddings-v4를 넘기기 위한 추가 개선이 필요하다. V2에서는 TomoroAI와 거의 근접했고, 일부 세부 태스크에서는 이미 앞서는 구간이 있다.
...
| Task | Current Best Ours (Run7) | jina-embeddings-v4 (3.8B) | TomoroAI (8B) |
|------|--------------------------|---------------------------|---------------|
| FinOCR | 0.6540 | 0.9410 | 0.8180 |
| MIR | 0.5461 | 0.7360 | 0.6090 |
| Office | 0.7400 | 0.8870 | 0.8420 |
| Slide | 0.7753 | 0.8970 | 0.8630 |
| VQA | 0.8422 | 0.8630 | 0.8290 |
| **Average** | **0.7115** | **0.8650** | **0.7920** |

### KoViDoRe V2 Per-Task Breakdown (NDCG@10)

| Task | Current Best Ours (Run7) | jina-embeddings-v4 (3.8B) | TomoroAI (8B) |
|------|--------------------------|---------------------------|---------------|
| Cybersecurity | 0.6489 | 0.7760 | 0.7370 |
| Economic | 0.2079 | 0.2450 | 0.1630 |
| Energy | 0.5498 | 0.6770 | 0.5850 |
| HR | 0.3291 | 0.5010 | 0.2650 |
| **Average** | **0.4339** | **0.5500** | **0.4380** |

## Technical Foundation

- **Current base model:** google/gemma-3-4b-it
...
| Aspect | Ours (Run7) | jina-embeddings-v4 | TomoroAI |
|--------|-------------|---------------------|----------|
| Base Model | Gemma3 4B | Qwen2.5-VL 3B | Qwen3-VL 8B + Qwen3-Embedding 8B |
...
| [Run7](run7_report.md) | colgemma3-ko-vdr-v0.9 | 0.7115 | 0.4339 | B200 환경, batch 128, 현재 최고 성능 |
...
- **아직 경쟁력 격차는 남아 있음:** V2 일부 태스크는 TomoroAI를 넘었지만, 전체 평균은 여전히 jina에 뒤처진다.
