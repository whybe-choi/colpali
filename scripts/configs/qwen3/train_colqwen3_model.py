"""
ColQwen3 Korean VDR Training Script

Base model: Qwen/Qwen3-VL (via colqwen3-base)
Loss: ColBERT cross-entropy with in-batch negatives (temperature=0.02)
LoRA target modules: q_proj, k_proj, v_proj, o_proj, gate_proj, up_proj, down_proj, custom_text_proj

Usage:
    CUDA_VISIBLE_DEVICES=0,1 torchrun --nproc_per_node=2 \
    scripts/configs/qwen3/train_colqwen3_model.py \
    --output-dir ./outputs/colqwen3-ko-run1 \
    --run-name colqwen3-ko-run1 \
...
def parse_args():
    ...
    # Model
    p.add_argument("--model-name-or-path", type=str, default="./models/base_models/colqwen3-base")
    ...
    p.add_argument(
        ...
        type=str,
        nargs="+",
        default=["q_proj", "k_proj", "v_proj", "o_proj", "gate_proj", "up_proj", "down_proj", "custom_text_proj"],
        help="LoRA target module suffixes. Pass as a space-separated list.",
    )
